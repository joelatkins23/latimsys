...
$to=$_POST['to'];
## Search 
$supplier_client_id=$_POST['supplier_client_id'];
$serach_text="";
if($supplier_client_id!=''){
    $serach_text.=" and a.supplier_id = '$supplier_client_id' or a.client_id = '$supplier_client_id' ";
}
$searchQuery = " ";
if($searchValue != ''){
...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a
                    ...
                    on a.agent_id=c.id 
                    where c.email='$email' 
                    AND  1  ".$serach_text." and a.branch='' AND 
                    DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' ");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a
                    left join agents c 
                    on a.agent_id=c.id 
                    where  1  ".$serach_text." and c.email='$email' 
                    AND a.branch='' ");   
    }
    ...
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a
                                left join agents c 
                                on a.agent_id=c.id 
                                where 1  ".$serach_text." and a.branch='' AND 
                                DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' ");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a
                                left join agents c 
                                on a.agent_id=c.id 
                                where  1  ".$serach_text." and a.branch='' ");   
    }  
...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        ...
        left join agents d on a.agent_id=d.id 
         WHERE 1  ".$serach_text." and d.email='$email' AND a.branch='' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."  ");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        left join accounts b on a.client_id =b.id 
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE  1  ".$serach_text." and d.email='$email' AND a.branch='' and 1 ".$searchQuery."");   
    }
    ...
    if ($to!='' && $from!='') {
        ...
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE 1  ".$serach_text." and a.branch='' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."  ");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        left join accounts b on a.client_id =b.id 
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE  1  ".$serach_text." and a.branch='' and 1 ".$searchQuery."");   
    }
...
if ($level=='Seller') { 
    ...
    if ($to!='' && $from!='') {
        $empQuery = " select a.id, a.fecha, a.status, a.service, a.tracking, b.name as customer_name, c.company as supplier_company,d.name as agent_name, a.customer_city from joborders a 
                    ...
                    left join accounts c on a.supplier_id =c.id 
                    left join agents d on a.agent_id=d.id 
                    WHERE   1 ".$serach_text." ".$searchQuery."  and a.branch='' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' 
                    order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
        ...
        $empQuery = " select a.id, a.fecha, a.status, a.service, a.tracking, b.name as customer_name, c.company as supplier_company,d.name as agent_name, a.customer_city from joborders a 
                    ...
                    left join accounts c on a.supplier_id =c.id 
                    left join agents d on a.agent_id=d.id 
                    WHERE   1 ".$serach_text." ".$searchQuery."  and a.branch=''                    
                    order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
    }
