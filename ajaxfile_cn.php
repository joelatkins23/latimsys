  ...
  if ($level=='Seller' && $type=='quick') { 
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a
                    ...
                    AND (a.status='PENDING' OR a.status='READY TO CONTACT' OR a.status='CHECK NOTES' OR a.status='IN PROCESS' OR a.status='SHIPPED' OR a.status='CANCELED' OR (a.status='IN WAREHOUSE' AND fecha > '2020-01-01') )");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a
                    left join agents c 
                    on a.agent_id=c.id 
...
if ($level=='Seller' && $type=='quick') { 
    if ($to!='' && $from!='') {
        ...
        left join agents d on a.agent_id=d.id 
         WHERE d.email='$email' AND a.branch='' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."  AND (a.status='PENDING' OR a.status='READY TO CONTACT' OR a.status='CHECK NOTES' OR a.status='IN PROCESS' OR a.status='SHIPPED' OR a.status='CANCELED' OR (a.status='IN WAREHOUSE' AND a.fecha > '2020-01-01') )");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        left join accounts b on a.client_id =b.id 
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE d.email='$email' AND a.branch='' and 1 ".$searchQuery."  AND (a.status='PENDING' OR a.status='READY TO CONTACT' OR a.status='CHECK NOTES' OR a.status='IN PROCESS' OR a.status='SHIPPED' OR a.status='CANCELED' OR (a.status='IN WAREHOUSE' AND a.fecha > '2020-01-01') )");   
    }
}elseif($level=='Seller' && $type=='all') {
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        ...
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE d.email='$email' AND a.branch='' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."  ");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        left join accounts b on a.client_id =b.id 
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE d.email='$email' AND a.branch='' and 1 ".$searchQuery."");   
    }
    ...
    if ($to!='' && $from!='') {
        ...
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE d.email='$email' AND a.branch='' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."  AND (a.status='IN WAREHOUSE')");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        left join accounts b on a.client_id =b.id 
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE d.email='$email' AND a.branch='' and 1 ".$searchQuery."  AND (a.status='IN WAREHOUSE')");   
    }    
}elseif($level!='Seller' && $type=='quick'){
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        ...
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE a.branch='' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."  AND (a.status='PENDING' OR a.status='READY TO CONTACT' OR a.status='CHECK NOTES' OR a.status='IN PROCESS' OR a.status='SHIPPED' OR a.status='CANCELED' OR (a.status='IN WAREHOUSE' AND a.fecha > '2020-01-01') )");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        left join accounts b on a.client_id =b.id 
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE a.branch='' and 1 ".$searchQuery."  AND (a.status='PENDING' OR a.status='READY TO CONTACT' OR a.status='CHECK NOTES' OR a.status='IN PROCESS' OR a.status='SHIPPED' OR a.status='CANCELED' OR (a.status='IN WAREHOUSE' AND a.fecha > '2020-01-01') )");   
    }
}elseif($level!='Seller' && $type=='all') {
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        ...
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE a.branch='' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."  ");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        left join accounts b on a.client_id =b.id 
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE a.branch='' and 1 ".$searchQuery."");   
    }
}elseif($level!='Seller' && $type=='warehouse') {  
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        ...
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE a.branch='' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."  AND (a.status='IN WAREHOUSE')");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        left join accounts b on a.client_id =b.id 
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE a.branch='' and 1 ".$searchQuery."  AND (a.status='IN WAREHOUSE')");   
    } 
}
...
if ($level=='Seller' && $type=='quick') { 
    ...
    if ($to!='' && $from!='') {
        $empQuery = " select a.id, a.fecha, a.status, a.service,a.atteched_files, a.tracking, b.name as customer_name, b.company as customer_company, c.company as supplier_company,d.name as agent_name, a.customer_city from joborders a 
                    ...
                    left join accounts c on a.supplier_id =c.id 
                    left join agents d on a.agent_id=d.id 
                    WHERE  a.branch='' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' 
                    and 1 ".$searchQuery." 
                    AND (a.status='PENDING' OR a.status='READY TO CONTACT' OR a.status='CHECK NOTES' OR a.status='IN PROCESS' OR a.status='SHIPPED' OR a.status='CANCELED' OR (a.status='IN WAREHOUSE' AND a.fecha > '2020-01-01') ) order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
