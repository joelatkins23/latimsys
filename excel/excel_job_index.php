...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        $sql = " select  a.fecha, a.id, b.name as customer_name, c.company as supplier_company, a.service, a.customer_city,d.name as agent_name, a.status from joborders a 
                    ...
                    WHERE  a.status='READY TO CONTACT' AND d.email='$email' 
  ...
  $columnHeader = "" . "\t" . "" . "\t" ."" . "\t" . "" ."READY TO CONTACT JOB ORDERS".$string."\t";  
  ...
  header("Content-Disposition: attachment; filename=JobOrders_ready_$fecha .xls");  
