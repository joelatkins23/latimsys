...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        ...
        left join agents d on a.agent_id=d.id 
         WHERE d.email='$email' AND a.branch='USA' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."  ");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        left join accounts b on a.client_id =b.id 
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE d.email='$email' AND a.branch='USA' and 1 ".$searchQuery."");   
    }
    ...
    if ($to!='' && $from!='') {
        ...
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE a.branch='USA' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."  ");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
        left join accounts b on a.client_id =b.id 
        left join accounts c on a.supplier_id =c.id 
        left join agents d on a.agent_id=d.id 
         WHERE  a.branch='USA' and 1 ".$searchQuery."");   
    }
...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        $empQuery = " select a.id, a.fecha, a.status, a.service, a.tracking, b.name as customer_name, c.company as supplier_company,d.name as agent_name, a.customer_city from joborders a 
                    ...
                    left join agents d on a.agent_id=d.id 
                    WHERE d.email='$email' 
                    AND a.branch='USA' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' 
                    and 1 ".$searchQuery." 
    ...
    if ($to!='' && $from!='') {
        $empQuery = " select a.id, a.fecha, a.status, a.service, a.tracking, b.name as customer_name, c.company as supplier_company,d.name as agent_name, a.customer_city from joborders a 
                    ...
                    left join accounts c on a.supplier_id =c.id 
                    left join agents d on a.agent_id=d.id 
                    WHERE a.branch='USA' AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' 
                    and 1 ".$searchQuery." 
                    order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
