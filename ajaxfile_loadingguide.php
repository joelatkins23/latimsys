...
if($searchValue != ''){
    ...
    foreach($filter_arr as $key=>$item1){    
        $accented_array = array('Š'=>'S', 'š'=>'s', 'Ž'=>'Z', 'ž'=>'z', 'À'=>'A', 'Á'=>'A', 'Â'=>'A', 'Ã'=>'A', 'Ä'=>'A', 'Å'=>'A', 'Æ'=>'A', 'Ç'=>'C', 'È'=>'E', 'É'=>'E','Ê'=>'E', 'Ë'=>'E', 'Ì'=>'I', 'Í'=>'I', 'Î'=>'I', 'Ï'=>'I', 'Ñ'=>'N', 'Ò'=>'O', 'Ó'=>'O', 'Ô'=>'O', 'Õ'=>'O', 'Ö'=>'O', 'Ø'=>'O', 'Ù'=>'U','Ú'=>'U', 'Û'=>'U', 'Ü'=>'U', 'Ý'=>'Y', 'Þ'=>'B', 'ß'=>'Ss', 'à'=>'a', 'á'=>'a', 'â'=>'a', 'ã'=>'a', 'ä'=>'a', 'å'=>'a', 'æ'=>'a', 'ç'=>'c','è'=>'e', 'é'=>'e', 'ê'=>'e', 'ë'=>'e', 'ì'=>'i', 'í'=>'i', 'î'=>'i', 'ï'=>'i', 'ð'=>'o', 'ñ'=>'n', 'ò'=>'o', 'ó'=>'o', 'ô'=>'o', 'õ'=>'o','ö'=>'o', 'ø'=>'o', 'ù'=>'u', 'ú'=>'u', 'û'=>'u', 'ý'=>'y', 'þ'=>'b', 'ÿ'=>'y' );
            ...
            a.all_total_pieces like '%".$item."%' or
            a.all_total_weight like '%".$item."%' or 
            a.all_total_volume like '%".$item."%' or 
            a.type like '%".$item."%' or 
            a.line like '%".$item."%' or 
...
// }else{
    $sel = mysqli_query($connect,"select count(*) as allcount FROM `loading_guide` a
...
// }else{
    $empQuery = "SELECT a.*,b.`name` AS agent_name FROM `loading_guide` a
...
while ($row = mysqli_fetch_assoc($empRecords)) {
        ...
        $action='<a href="#" style="color:red" onclick="editloadingguide('.$row['id'].')"><i class="fa fa-edit action"></i></a>';
