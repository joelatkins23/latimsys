...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
                    ...
                    where d.email='$email' 
                    AND a.branch='' 
                    AND (b.name LIKE '%$search_val%' OR c.company LIKE '%$search_val%' OR  a.id LIKE '%$search_val%') 
                    AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' ");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
                        left join accounts b on a.client_id =b.id 
                        left join accounts c on a.supplier_id =c.id 
...
if ($level=='Seller') {
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
                    ...
                    AND a.branch='' 
                    AND (b.name LIKE '%$search_val%' OR c.company LIKE '%$search_val%' OR  a.id LIKE '%$search_val%') 
                    AND DATE_FORMAT(a.fecha,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery."");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders a 
                        left join accounts b on a.client_id =b.id 
                        left join accounts c on a.supplier_id =c.id 
                        ...
                        where d.email='$email' 
                        AND a.branch='' 
                        AND (b.name LIKE '%$search_val%' OR c.company LIKE '%$search_val%' OR  a.id LIKE '%$search_val%') and 1 ".$searchQuery."");
    }  
